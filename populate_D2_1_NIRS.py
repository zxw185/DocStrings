...
def getData(wl1Path, wl2Path, hdrPath):
    """Retrieves and returns experimental data and metadata."""
...
def getFilePaths():
    """Retrieves all filepaths from directory and returns lists containing respective paths."""
    ...
    for i in range(1, 44):  # Appends all paths to a single list
    ...
    for path in filepaths:  # Appends filepaths to respective lists
        if ".wl1" in path:
...
def insertData(wl1Path, wl2Path, hdrPath, db_name, db_user, db_password, experimentalunitnumber, endpointnumber,
               treatmentnumber, DataSourceNumber):  # Think datasourcenumber is moved in new version?
